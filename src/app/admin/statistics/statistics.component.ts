...
export class StatisticsComponent implements OnInit {
  ...
  formatLuck(difficulty, shares) {
    const percent = Math.round(shares / difficulty * 100);
    if (!percent) {
      return '?';
    }
    return percent + '%';
  ...
  getStats(promptPassword) {
    ...
    let password = this.getCookiesItem('password');
    ...
    this.dataService.getStatus().subscribe(status => {
      ...
      if (Object.keys(status).length > 0) {
        ...
        this.dataService.getAdminStatus(password).subscribe(data => {
  ...
  updateStatus() {
    ...
    this.data.lastProfit = this.dataService.getReadableCoins(this.status,
      (this.data.totalRevenue - this.data.totalOwed - this.data.totalPaid), 4);
